...
**Headline (historical): this 54-endpoint matrix dates from the RobotWare 7.21-only era of the extension. The
authoritative coverage record now lives in the client: `abb-rws-client/CONFORMANCE.md` (what the live
controllers advertise vs what the client implements, regenerated by `npm run conformance`) and the live-tested
matrix RobotWare 8.1 / 7.21 (OmniCore VCs) and 6.16 (IRC5 VC) in `abb-rws-client/README.md`.**
